...
describe('secret generation', () => {
  it('creates a 256-bit OAuth state', () => {
    ...
    expect(state).toMatch(/^swpo2\.[A-Za-z0-9_-]{43}$/);
  ...
  it('creates a signed setup proof that expires after ten minutes', async () => {
    ...
    expect(proof).toMatch(
      /^swps2\.[A-Za-z0-9_-]{22}\.\d{13}\.[A-Za-z0-9_-]{43}$/
    );
    await expect(classifySetupProof(proof, pairingKey, nowMs)).resolves.not.toBeNull();
    await expect(classifySetupProof(proof, pairingKey, expiresAtMs + 1)).resolves.toBeNull();
    await expect(
      classifySetupProof(
        `${proof.slice(0, -1)}${proof.endsWith('A') ? 'Q' : 'A'}`,
        pairingKey,
        nowMs
      )
    ).resolves.toBeNull();
...
describe('AES-256-GCM storage encryption', () => {
  ...
  it('rejects plaintext that cannot fit the encrypted database field', async () => {
    const withinLimit = 'a'.repeat(MAX_ENCRYPTED_SECRET_PLAINTEXT_BYTES);
    const overLimit = 'a'.repeat(MAX_ENCRYPTED_SECRET_PLAINTEXT_BYTES + 1);

    await expect(
      encryptSecret(withinLimit, context, 'current', encryptionKeys)
    ).resolves.toMatchObject({ keyId: 'current' });
    await expect(
      encryptSecret(overLimit, context, 'current', encryptionKeys)
