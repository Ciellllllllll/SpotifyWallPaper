...
import { discardBoundedBody, readBoundedText } from './http.js';
...
export interface SpotifyDependencies {
...
export async function sendSpotifyCommand(
  ...
  const request = commandRequest(command, endpoint);
  let response: Response;
  try {
...
async function retryPlaybackAfterUnauthorized(
  ...
  const current = await getCredentialByPublicId(db, credential.publicId);
  await invalidateAccessToken(
    db,
    credential.publicId,
    current?.tokenVersion ?? credential.tokenVersion,
    nowMs
  );
...
async function refreshAccessToken(
  ...
  try {
    ...
    env.recordRefresh?.(refreshMetricOutcome(result));
    return result;
  } catch {
    env.recordRefresh?.('failed');
    return unavailable();
  } finally {
    try {
      await releaseRefreshLease(
      ...
      // Lease expiry remains the final recovery path if PostgreSQL is unavailable.
    }
  }
}
...
async function runRefreshAccessToken(
  ...
  const keyring = env.encryptionKeyring;
  ...
  let response: Response;
  try {
    response = await (options.fetcher ?? fetch)(env.tokenEndpoint, {
      method: 'POST',
      headers: {
...
function commandRequest(command: PlaybackCommand, endpoint: string): {
  method: 'POST' | 'PUT';
  url: string;
} {
