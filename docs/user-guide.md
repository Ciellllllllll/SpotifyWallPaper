...
更新結果の保存に失敗した場合、元のセッションは受け取った応答を保持し、次の要求時に先に保存を再試行します。
同じ保存領域を使う別セッションは、未確定の更新を期限切れだけで再送しません。
元のプロセスが終了して応答を失った場合や、Token更新通信が途切れて結果を確認できない場合は、保存領域を復旧しPagesから再認証してください。
PC移行、壁紙の配置先変更、ブラウザデータ削除では保存領域が変わり、再認証が必要になる場合があります。

### 複数画面と長時間動作

同一保存領域ではIndexedDBの短い原子的更新とlease・revisionで競合を制御します。leaseの待機期限を過ぎても、未確定の更新は再送しません。
