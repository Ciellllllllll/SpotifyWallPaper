...
`spotify_client_id`, `spotify_refresh_token`, and `settings_json` use Wallpaper Engine text input fields. Paste
`settings_json` as single-line JSON. Do not use real Refresh Tokens in screenshots, recordings, or public QA notes
because the input field can be visible; use dummy values when checking the field itself.
