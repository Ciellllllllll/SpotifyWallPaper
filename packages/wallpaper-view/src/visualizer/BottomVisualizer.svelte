...
<style>
  .visualizer { position: absolute; display: grid; width: 100%; height: 100%; place-items: center; overflow: visible; pointer-events: none; color: var(--visualizer-color, #ffffff); opacity: .84; filter: drop-shadow(0 0 calc(18px * var(--visualizer-glow, 0)) var(--visualizer-color, #ffffff)); }
  ...
  .bottom-bar-glow { fill: var(--visualizer-color, #ffffff); opacity: .28; filter: blur(1px); }
  ...
  .peak-band-glow { fill: var(--visualizer-color, #ffffff); opacity: .3; filter: blur(1px); }
